...
const TaskLists: React.FC<Props> = ({ todos, onRemove, onEditStart }) => {
    ...
    return (
        <ul className="space-y-3 max-h-64 sm:max-h-80 overflow-y-auto pr-2 scroll-smooth scrollbar-thin scrollbar-thumb-blue-300 scrollbar-track-blue-100">
            {todos?.map(({ id, text }) => (
                <li
                    key={id}
                    className="flex justify-between items-center text-slate-800 w-full rounded-md p-3 transition-all hover:bg-slate-100 focus:bg-slate-100 active:bg-slate-100 shadow-sm"
                >
